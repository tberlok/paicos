...
class subfind_catalog:
  def __init__(self, basedir, snapnum, verbose=False):    
    if os.path.exists(basedir + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + ".hdf5"):
      ...
      self.first_file_name = basedir + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + ".hdf5"
    elif os.path.exists(basedir + "/groups_" + str(snapnum).zfill(3) + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + ".0.hdf5"):
      self.multi_file = True
      self.first_file_name = basedir + "/groups_" + str(snapnum).zfill(3) + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + ".0.hdf5"
      self.no_groupdir = False
    elif os.path.exists(basedir + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + ".0.hdf5"):
      self.multi_file = True
      self.first_file_name = basedir + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + ".0.hdf5"
    ...
    for ifile in range(self.nfiles):
      if self.multi_file == False:
        ...
        if self.no_groupdir==False:
          cur_filename = basedir + "/groups_" + str(snapnum).zfill(3) + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + "."+str(ifile)+".hdf5"
        else:
          cur_filename = basedir + "/fof_subhalo_tab_" + str(snapnum).zfill(3) + "."+str(ifile)+".hdf5"
